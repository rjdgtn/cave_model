...
wasInited(false),
shouldConvertToExtendedElevation(false),
colourMult(1.0f),
...
void Cave::finishInit(OutputType enabledToGenerateOutput) {
	...
	if (!shouldConvertToExtendedElevation) buildFakeZSurveyPikets();
...
void Cave::genPiketsFakeWallsСhainSearch(FakeWallsMode mode, Piket* beginPiket, Piket* curEnd, int accumLength, std::vector<Piket*>& intermPikets) {
    ...
    for (int i = 0; i < curEnd->adjPikets.size(); i++) {
        ...
        if (beginPiket == piket) continue; // loop
        if (beginPiket->isInactive()) continue; // zsurvey
        if (find(intermPikets.begin(), intermPikets.end(), piket) != intermPikets.end()) continue; // loop

        if (!piket->classifiedWalls.empty()) {
            if (!intermPikets.empty()) genPiketsFakeWallsProcessChain(mode, beginPiket, intermPikets, piket);    
        } else {
            if (intermPikets.size() >= caveViewPrefs.stretchWallsOverPiketNumMax) break; // too long walls stretch
...
void Cave::genPiketsFakeWallsBudge(Piket* beginPiket, std::vector<Piket*> intermPikets, Piket* endPiket) {
    ...
    if (beginPiketPos == endPiketPos) return;
    ...
    for(int i = 0; i < intermPikets.size(); i++) {
        ...
        if (i == 0) prevPiket = beginPiket;
        else prevPiket = intermPikets[i-1];
        
        const Piket* nextPiket;
        if (i == intermPikets.size() - 1) nextPiket = endPiket;
        else nextPiket = intermPikets[i+1];
...
//            debugDraw(projCurToEdgeLine, edgeBegin);
        ...
        if (wallsPolygon2d.size() < 3) return;
...
void Cave::genPiketsFakeWallsTrail(Piket* beginPiket, std::vector<Piket*> intermPikets, Piket* endPiket) {
    ...
    for(int i = 0; i < intermPikets.size(); i++) {
        if (i == 0) totalDist += (intermPikets[i]->pos - beginPiket->pos).length();
        else totalDist += (intermPikets[i]->pos - intermPikets[i-1]->pos).length();
    ...
    for (int i = 0; i < buildWalls.size(); i++) {
        ...
        bool aFromBeginPiket = (std::find_if(beginPiket->classifiedWalls.begin(), beginPiket->classifiedWalls.end(), ComareObjByAdr<PiketWall>(trgl.a)) != beginPiket->classifiedWalls.end());
        bool bFromBeginPiket = (std::find_if(beginPiket->classifiedWalls.begin(), beginPiket->classifiedWalls.end(), ComareObjByAdr<PiketWall>(trgl.b)) != beginPiket->classifiedWalls.end());
        bool cFromBeginPiket = (std::find_if(beginPiket->classifiedWalls.begin(), beginPiket->classifiedWalls.end(), ComareObjByAdr<PiketWall>(trgl.c)) != beginPiket->classifiedWalls.end());

        if (aFromBeginPiket) {
            if (!bFromBeginPiket) wallPairs[trgl.a].insert(trgl.b); 
            if (!cFromBeginPiket) wallPairs[trgl.a].insert(trgl.c);  
        }
        if (bFromBeginPiket) {
            if (!aFromBeginPiket) wallPairs[trgl.b].insert(trgl.a); 
            if (!cFromBeginPiket) wallPairs[trgl.b].insert(trgl.c);  
        } 
        if (cFromBeginPiket) {                                     
            if (!aFromBeginPiket) wallPairs[trgl.c].insert(trgl.a);
            if (!bFromBeginPiket) wallPairs[trgl.c].insert(trgl.b);  
        }
    }
...
//                point3df edgeEndModP3 = intersection_point<float>(edgeLine, endPiketNormalPlane);
                ...
                if (std::max<float>(posToBeginDist, posToEndDist) > beginToEndDistance){
                    if (posToBeginDist < posToEndDist) pos = edgeBegin;
                    else pos = edgeEnd; 
                }                                          
...
void Cave::addEdge(int verticeId0, int verticeId1) {
	...
	if (!piket1) LOG("fail to add edge: piket " << verticeId1 << " not found ");
...
void Cave::buildThreadObject() {
	...
	for (; itFrom != pikets.end(); itFrom++) {
		...
		for (; itTo != pFrom.adjPikets.end(); itTo++) {   
			...
			if (pFrom.id < pTo.id) {
				addOutputLine(OT_THREAD, pFrom.pos, pTo.pos, pFrom.getPrevailWallColor(), pTo.getPrevailWallColor());
			}	
		}
	}
}
...
void Cave::prebuildPikets() {
	if (prebuildInvalidated) {
		...
		if (shouldConvertToExtendedElevation) convertToExtendedElevation();
	}
}
...
//					debug2->position(nextPiket->classifiedWalls[widx].pos);
            ...
            for (int wallPairIdx = 0; wallPairIdx < wallPairs.size(); wallPairIdx++) {
                ...
                if (!trgl.clockwise) normal = -normal;
...
//                debugDraw(trgl.c->pos, trgl.c->pos + normal.normalisedCopy() * 300 * cWeight, Color(1, 0, 1, 1));
            ...
            if (!wallPairs.empty()) segmentDraw++; 
        }
    }
...
//    for (int i = 1; i < 3/*wallTriangles.size()*/; i++ ) { 
    for (int i = 0; i < wallTriangles.size(); i++ ) {
        ...
        if (triangle.clockwise == CLOCKWISE) {
            drawTriangleSmooth3(triangulationSmooth
                              , triangle.ca
                              , triangle.cb
                              , triangle.cc
                              , triangle.a->pos, wallNormals[triangle.a].get()
                              , triangle.b->pos, wallNormals[triangle.b].get()
                              , triangle.c->pos, wallNormals[triangle.c].get());
        } else {
            drawTriangleSmooth3(triangulationSmooth
                              , triangle.ca
                              , triangle.cc
    ...
    for (int i = 0; i < smoothedTriangles.size(); i++ ) {
        ...
        if (vt.clockwise) lastTriangles.push_back(SurfaceTriangle(vt.ca, vt.cb, vt.cc, vt.a, an, vt.b, bn, vt.c, cn));
        else lastTriangles.push_back(SurfaceTriangle(vt.ca, vt.cc, vt.cb, vt.a, an, vt.c, cn, vt.b, bn));
    }    
...
//	addOutputLine(OT_WALL_LINES, a, b, ca/2, cb/2);
//	wallsLines->position(a);
//	wallsLines->colour(ca / 2);
//	wallsLines->position(b);
//	wallsLines->colour(cb / 2);

//	addOutputLine(OT_WALL_LINES, b, c, cb/2, cc/2);
//	wallsLines->position(b);
//	wallsLines->colour(cb / 2);
//	wallsLines->position(c);
//	wallsLines->colour(cc / 2);

//	addOutputLine(OT_WALL_LINES, c, a, cc/2, ca/2);
//	wallsLines->position(c);
//	wallsLines->colour(cc / 2);
//	wallsLines->position(a);
//	wallsLines->colour(ca / 2);
