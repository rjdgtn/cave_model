...
protected:
    ...
    std::vector<PiketWall> propagateWallAngleAbove(int wallId1, int wallId2, int num, WallsBlowMode blowMode ); // алгорим размножения стен на основе угламежду ими и центром
    std::vector<PiketWall> propagateWallBesier3(int h, int i, int j, int k, int addWallsNum, float strong = 0.4f); // алгоритм размножения стен на основе безье 3 порядка
